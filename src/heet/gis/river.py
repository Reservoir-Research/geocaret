...
data_sources_file = load_packaged_data('./config/data_sources.yaml')
with open(data_sources_file, "r", encoding="utf8") as file:
...
class River:
    """ Class and methods for manipulating EE River geometries. """
    def __init__(self, river_feature: FeatureData):
        self.river = river_feature

    @staticmethod
    def pts_to_linestring(coords) -> ee.Feature:
        """ """
        pt1_coord_list = ee.List(ee.List(coords).get(0))
        pt2_coord_list = ee.List(ee.List(coords).get(1))
        # Create a start point
        pt_start = ee.Geometry.Point(
            ee.Number(pt1_coord_list.get(0)),
            ee.Number(pt1_coord_list.get(1)))
        # Create an end point
        pt_end = ee.Geometry.Point(
            ee.Number(pt2_coord_list.get(0)),
            ee.Number(pt2_coord_list.get(1)))
        # Create an output line Geometry
        output_line = ee.Geometry.LineString(
            ee.List([pt_start, pt_end]))
        # Convert line geometry to Feature and add point attributes
        feature = ee.Feature(output_line)
        feature = feature.set('pt_start', pt_start)
        feature = feature.set('pt_end', pt_end)
        return feature

    @staticmethod
    def polygon_to_lines(poly_ftc: ee.FeatureCollection) -> ee.FeatureCollection:
        """ """
        logger.debug(
            f"[River.polygon_to_lines] Input Polygon feature collection: {poly_ftc.getInfo()}")
        # Convert polygon to a collection of points
        poly_points_list = poly_ftc.first().geometry().coordinates().get(0)
        logger.debug(
            f"[River.polygon_to_lines] Polygon Points List: {poly_points_list.getInfo()}")
        ...
        # NOTE: Changed from .get(-1) (last pt) to .get(0)
        poly_end_pts_list = (ee.List(poly_points_list).slice(1, None, 1).add(
            ee.List(poly_points_list).get(0)))
        poly_line_pts_list = poly_start_pts_list.zip(poly_end_pts_list)
        logger.debug(
            f"[River.polygon_to_lines] Polygon Start Points List: {poly_start_pts_list.getInfo()}")
        logger.debug(
            f"[River.polygon_to_lines] Polygon End Points List: {poly_end_pts_list.getInfo()}")
        logger.debug(
            f"[River.polygon_to_lines] Polygon Line Coords List: {poly_line_pts_list.getInfo()}")
        poly_boundaries_ftc = ee.FeatureCollection(
            poly_line_pts_list.map(River.pts_to_linestring))
        poly_boundaries_ftc = poly_boundaries_ftc.map(
            lambda rfeat: River.remove_property(rfeat, 'pt_start'))
        poly_boundaries_ftc = poly_boundaries_ftc.map(
            lambda rfeat: River.remove_property(rfeat, 'pt_end'))
        logger.debug(
            f"Polygon Boundary Lines FTC: {poly_boundaries_ftc.getInfo()}")
        return poly_boundaries_ftc

    @staticmethod
    def remove_property(feature: ee.Feature, property_name: str) -> ee.Feature:
        """Generic Function to remove a property from a feature."""
        selected_properties = feature.propertyNames().filter(
            ee.Filter.neq('item', property_name))
        return feature.select(selected_properties)


class HydroRivers(River):
    """ """
    def __init__(self, river_feature: HydroRiversData):
        super().__init__(river_feature)




river = HydroRivers(river_feature=HydroRiversData.fromsource(
    source=data_sources['hydrorivers']['url']))
...
def filter_main(hyriv_id, hydrorivers_extract):
    ...
    main_dam_reach_ft = (hydrorivers_extract
                         .filter(ee.Filter.eq('HYRIV_ID', hyriv_id)))
    ...
    main_hydrorivers = (hydrorivers_extract
                        .filter(ee.Filter.eq('ORD_CLAS', dam_ord_clas)))
...
def find_main_channel(inundated_river_ftc, damFeat):
    ...
    main_dam_reach_ft = (inundated_river_ftc
                         .filterBounds(damFeat.geometry())
                         .sort('DIS_AV_CMS', False)
                         .first())
    logger.debug(f"[delineate_river] main_dam_reach_ft: {main_dam_reach_ft.getInfo()}")
    main_dam_reach_id = main_dam_reach_ft.get('HYRIV_ID')
    logger.debug(f"[delineate_river] main_dam_reach_id: {main_dam_reach_id.getInfo()}")
    inundated_river_main_ftc = filter_main(
        main_dam_reach_id, inundated_river_ftc)
    logger.debug(f"[delineate_river] inundated_river_ftc: {inundated_river_main_ftc.getInfo()}")
...
def inundated_reaches(sres_ftc):
    ...
    def is_source(rfeat):
        ...
        is_source = ee.Algorithms.If(
            ...
            ee.List(next_down_set).contains(t_hyriv_id), 0, 1)
    ...
    def is_sink(rfeat):
        ...
        is_sink = ee.Algorithms.If(
            ...
            ee.List(hyriv_id_set).contains(t_next_down), 0, 1)
...
def delineate_river(damFeat, res_ftc, c_dam_id_str):
    ...
    sres_boundaries_ftc = River.polygon_to_lines(sres_ftc)

    # ==========================================================================
    # Export simplified reservoir line boundary
    # ==========================================================================

    if config['export_simplified_reservoir_boundary']:
    ...
    logger.debug(f"[delineate_river] reaches_sres_ftc: {reaches_sres_ftc.getInfo()}")
    ...
    def has_boundary_pts(rfeat):
        ...
        boundary_pts = sres_boundaries_ftc.toList(1000000).map(
            ...
            # Intersects
                ...
                # Intersection
                    ee.Feature(res_geom).intersection(
                        rfeat, ee.ErrorMargin(1)), 999))

        nbpts = ee.List(boundary_pts).filter(
            ee.Filter.neq('item', 999)).length()
        rfeat = rfeat.set('nbpts', nbpts)
        return rfeat

    reaches_sres_sas_ftc = (
        reaches_sres_ftc
        .filter(ee.Filter.eq('is_source', 1))
    ...
    reaches_sres_sink_ftc = (
        reaches_sres_ftc
        .filter(ee.Filter.eq('is_source', 0))
        .filter(ee.Filter.eq('is_sink', 1))
    ).map(has_boundary_pts)
    ...
    logger.debug(f"[delineate_river] reaches_sres_iolet_ftc: {reaches_sres_iolet_ftc.getInfo()}")

    # Simple terminal reaches
    reaches_sres_iolet_ssas_ftc = (
        reaches_sres_iolet_ftc
        .filter(ee.Filter.eq('is_source', 1))
        .filter(ee.Filter.eq('is_sink', 1))
    ...
    logger.debug(f"[delineate_river] ssas: {reaches_sres_iolet_ssas_ftc.getInfo()}")
    logger.debug(f"[delineate_river] ssource: {reaches_sres_iolet_ssource_ftc.getInfo()}")
    logger.debug(f"[delineate_river] ssink: {reaches_sres_iolet_ssink_ftc.getInfo()}")
    logger.debug(f"[delineate_river] csas: {reaches_sres_iolet_csas_ftc.getInfo()}")
    logger.debug(f"[delineate_river] csource: {reaches_sres_iolet_csource_ftc.getInfo()}")
    logger.debug(f"[delineate_river] csink: {reaches_sres_iolet_csink_ftc.getInfo()}")
    logger.debug(f"[delineate_river] other: {inundated_river_other_ftc.getInfo()}")
    ...
    def reach_to_lines_source_ftc(rfeat):
        ...
        reach_lines_ftc = ee.FeatureCollection(
            reach_line_pts_list.map(River.pts_to_linestring)
        )

        def is_iolet(geom_line):
            """ """
            has_iolet = sres_boundaries_ftc.toList(1000000).map(lambda res_geom:
                                                                ee.Algorithms.If(
                                                                    ee.Feature(res_geom).intersects(
                                                                        geom_line, ee.ErrorMargin(1)), 1, 0
                                                                )
                                                                )
        ...
        def has_boundary_pts(geom_line):
            ...
            boundary_pts = sres_boundaries_ftc.toList(1000000).map(lambda res_geom:
                                                                   ee.Algorithms.If(
                                                                       # Intersects
                                                                       ee.Feature(res_geom).intersects(
                                                                           geom_line, ee.ErrorMargin(1)),
                                                                       # Intersection
                                                                       ee.Feature(res_geom).intersection(
                                                                           geom_line, ee.ErrorMargin(1)),
                                                                       999
        ...
        if debug_mode:
            ...
            print("[reach_to_lines_ftc] Reach Points List", reach_points_list.getInfo(), "\n")
            print("[reach_to_lines_ftc] Reach Start Points List",
                  reach_start_points_list.getInfo(), "\n")
            print("[reach_to_lines_ftc] Reach End Points List",
                  reach_end_points_list.getInfo(), "\n")
            print("[reach_to_lines_ftc] Reach Line Coords List",
        ...
        def truncate_source_line(lines_iolet_ftc):
           ...
           # Identify Line Segments Outside Reservoir to Discard
            ...
            if debug_mode:
                print("[DEBUG] [truncate_source_line] lines_iolet_list",
                      lines_iolet_list.getInfo(), "\n")
                print("[DEBUG] [truncate_source_line] iolet_list", iolet_list.getInfo(), "\n")
                print("[DEBUG] [truncate_source_line]", retain_from_index.getInfo(), "\n")

            # Identify the line Segment that Needs to be Trimmed
            line_to_trim_ft = ee.Feature(
                lines_iolet_list.slice(retain_from_index, ee.Number(
                    retain_from_index).add(1), 1).get(0)
            )
            ...
            boundary_pts_filtered_ftc = (
                ...
                .filter(ee.Filter.lte('updist', 1e10))
                .sort('updist', True)
            )

            npt_start = ee.FeatureCollection(boundary_pts_filtered_ftc).first().geometry()

            if debug_mode:
                print(" [truncate_source_reach] Boundary Pts", boundary_pts_ftc.getInfo())
                print(" [truncate_source_reach] Filtered Boundary Pts",
                      boundary_pts_filtered_ftc.getInfo())
                print(" [truncate_source_reach] In Points", npt_start.getInfo(), pt_end.getInfo())
            ...
            if debug_mode:
                print(" [truncate_source_reach] Trimmed Source Line",
                      trimmed_line_ftc.getInfo(), "\n")
                print(" [truncate_source_reach] Selected Source Lines List",
                      selected_lines_list.getInfo(), "\n")
                print(" [truncate_source_reach] Selected Source Lines FTC",
    ...
    def reach_to_lines_sink_ftc(rfeat):
        ...
        def has_boundary_pts(geom_line):
            """ """
            boundary_pts = sres_boundaries_ftc.toList(
                1000000).map(lambda res_geom: ee.Algorithms.If(
                # Intersects
                ee.Feature(res_geom).intersects(geom_line, ee.ErrorMargin(1)),
                # Intersection
                ee.Feature(res_geom).intersection(
                    geom_line, ee.ErrorMargin(1)), 999))
            boundary_pts = ee.List(boundary_pts).filter(
        ...
        def truncate_sink_line(lines_iolet_ftc):
            ...
            if debug_mode:
                print("[DEBUG] [truncate_sink_line] lines_iolet_list",
                      lines_iolet_list.getInfo(), "\n")
                print("[DEBUG] [truncate_sink_line] iolet_list", iolet_list.getInfo(), "\n")
            ...
            if debug_mode:
                print("[DEBUG] [truncate_sink_line] line_to_trim_ft",
                      line_to_trim_ft.getInfo(), "\n")

            # Identify line segments upstream of the one to be trimmed
            # (to be kept)
            selected_lines_list = lines_iolet_list.slice(0, ee.Number(retain_to_index), 1)

            if debug_mode:
                print("[DEBUG] [truncate_sink_line] selected_lines_list",
                      selected_lines_list.getInfo(), "\n")
            ...
            if debug_mode:
                print("[DEBUG] [truncate_sink_line] pt_start, pt_end, ",
                      pt_start.getInfo(),  pt_end.getInfo(), "\n")
            ...
            if debug_mode:
                ...
                print(" [truncate_sink_reach] Filtered Boundary Pts",
                      boundary_pts_filtered_ftc.getInfo())
                print(" [truncate_sink_reach] In Points", pt_start.getInfo(), npt_end.getInfo())
            ...
            if debug_mode:
                print("[DEBUG] [truncate_sink_line] trimmed_line_geom",
    ...
    if debug_mode:
        ...
        t_inundated_river_csas_ftc = reaches_sres_iolet_csas_ftc.map(reach_to_lines_source_ftc)
        inundated_river_csas_ftc = t_inundated_river_csas_ftc.map(reach_to_lines_sink_ftc)
    ...
    if debug_mode:
        ...
        inundated_river_csink_ftc = reaches_sres_iolet_csink_ftc.map(reach_to_lines_sink_ftc)

    if debug_mode:
        print("[DEBUG] Sink")
        print("[DEBUG] inundated_river_sink_ftc", inundated_river_csink_ftc.getInfo(), "\n")

   # ==========================================================================
   # [2-a Optional] Export river sink reaches
   # ==========================================================================

    if config['export_sink_lines']:
        msg = "Exporting sink lines"

        try:
            logger.info(f'{msg} {c_dam_id_str}')
            heet.export.export_ftc(inundated_river_csink_ftc, c_dam_id_str, "river_sink_lines")
    ...
    if debug_mode:
        ...
        inundated_river_csource_ftc = reaches_sres_iolet_csource_ftc.map(reach_to_lines_source_ftc)

    if debug_mode:
        print("[DEBUG] Source")
        print("[DEBUG] inundated_river_source_ftc", inundated_river_csource_ftc.getInfo(), "\n")

   # ==========================================================================
   # [3-a Optional] Export river source lines
   # ==========================================================================

    if config['export_source_lines']:
        msg = "Exporting source lines"

        try:
            logger.info(f'{msg} {c_dam_id_str}')
            heet.export.export_ftc(inundated_river_csource_ftc, c_dam_id_str, "river_source_lines")
    ...
    if debug_mode:
        print("[DEBUG]")
        print("[DEBUG][delineate_river] Pre-flatten inundated_river_ftc",
              inundated_river_cftc.getInfo(), "\n")
        print("[DEBUG][delineate_river] inundated_river_ftc", inundated_river_ftc.getInfo(), "\n")
...
def batch_delineate_rivers(c_dam_ids):
    ...
    for c_dam_id in c_dam_ids:
        ...
        reservoir_asset_name = assets.ps_heet_folder + "/" + "R_" + c_dam_id_str
        res_ftc = ee.FeatureCollection(reservoir_asset_name)

        snapped_asset_name = assets.ps_heet_folder + "/" + "PS_" + c_dam_id_str
        dam_feat = ee.FeatureCollection(snapped_asset_name)

        river_vector, main_river_vector = delineate_river(
            dam_feat, res_ftc, c_dam_id_str)
        ...
        if config['export_river']:
            ...
            try:
                logger.info(f' {msg} {c_dam_id_str}')
                heet.export.export_ftc(river_vector, c_dam_id_str, "river_vector")
        ...
        if config['export_main_river']:
            ...
            try:
                logger.info(f' {msg} {c_dam_id_str}')
                heet.export.export_ftc(
                    main_river_vector, c_dam_id_str, "main_river_vector")
...
if __name__ == "__main__":
    ...
    reservoir_asset_name = \
        "users/KamillaHarding/XHEET/AFRICA2_20220222-1303/R_3077"
    res_ftc = ee.FeatureCollection(reservoir_asset_name)
    snapped_asset_name = \
        "users/KamillaHarding/XHEET/AFRICA2_20220222-1303/PS_3077"
    dam_feat = ee.FeatureCollection(snapped_asset_name)
    main_river_vector, river_vector = delineate_river(
        dam_feat, res_ftc, "3130")
