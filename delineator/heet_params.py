...
def delineate_buffer_zone(res_ftc):
    ...
    buffer_distance = ee.Number(res_area_m.divide(ee.Number(3.14159265359)).pow(0.5)).divide(2)

    def add_buffer(feat):
      return feat.buffer(buffer_distance)
         
    res_buffered_ftc = res_ftc.map(add_buffer)
    buffer_zone_ft = ee.Feature(res_buffered_ftc.geometry().difference(res_ftc.geometry(),1))
...
def degrees_to_perc_slope(mean_slope_degrees_value):
    return (ee.Number(mean_slope_degrees_value)
...
def landcover_bysoil_buffer(res_ftc, landcover_analysis_file_str, c_dam_id):
    ...
    try:
        ...
        if cfg.exportBufferVector == True:
            heet_export.export_ftc(res_buffer_ftc, c_dam_id_str, "reservoir_buffer_zone")
...
def maximum_depth(reservoir_ftc):
    ...
    water_level_elevation = ee.Number.parse(reservoir_ftc.first().get('r_imputed_water_elevation'))
...
def profile_catchment(catchment_ftc, landcover_analysis_file_str):
    ...
    if debug_mode == True:
        print("[DEBUG] [profile_catchment] mean_slope_pc", mean_slope_pc.getInfo(),"\n")
        
    # Landcover, proportions, European Space Agency
    landcover_fracs = landcover(catchment_ftc, landcover_analysis_file_str).map(lambda value: ee.Number(value).format("%.3f"))

    if debug_mode == True:
        print("[DEBUG] [profile_catchment] landcover_fracs", landcover_fracs.getInfo(),"\n")
    ...
    msocc_perc = ee.Algorithms.If(
        ee.Number(t_msocc_perc).neq(-999),
        ee.Number(t_msocc_perc).format("%.3f"),
        "ND"
    )
...
def profile_reservoir(reservoir_ftc, landcover_analysis_file_str, c_dam_id):
    ...
    if c_dam_id in mtr.buffer_method:
        landcover_bysoil_fracs = landcover_bysoil_buffer(reservoir_ftc, landcover_analysis_file_str, c_dam_id).map(lambda value: ee.Number(value).format("%.3f"))
    else:
        landcover_bysoil_fracs = landcover_bysoil(reservoir_ftc, landcover_analysis_file_str).map(lambda value: ee.Number(value).format("%.3f"))
...
def batch_profile_reservoirs(c_dam_ids):
    ...
    for c_dam_id in c_dam_ids:
        ...
        try:
            landcover_analysis_file_str = str(mtr.id_landcover_analysis_file_lookup[c_dam_id]) 
            updated_reservoir_ftc = profile_reservoir(reservoir_ftc, landcover_analysis_file_str, c_dam_id)
...
def batch_profile_catchments(c_dam_ids):
    ...
    for c_dam_id in c_dam_ids:
        ...
        try:
            landcover_analysis_file_str = str(mtr.id_landcover_analysis_file_lookup[c_dam_id]) 
            updated_catchment_ftc = profile_catchment(catchment_ftc, landcover_analysis_file_str)
