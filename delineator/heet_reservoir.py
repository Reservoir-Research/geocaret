...
def batch_delineate_reservoirs(c_dam_ids) :
    ...
    for c_dam_id in c_dam_ids:  
        ...
        if c_dam_id in mtr.existing_dams:
            landcover_delineation_file_str = str(mtr.id_landcover_delineation_file_lookup[c_dam_id]) 
            reservoirVector= delineate_existing_reservoir(catchmentVector, c_dam_id_str, landcover_delineation_file_str)
        else: 
...
def delineate_future_reservoir(catchmentVector, c_dam_id_str):
    ...
    # Dam location
    snapped_dam_point_location = ee.Geometry.Point(snapped_dam_longitude, snapped_dam_latitude)
    ...
    mad_m3_pers = ee.Number(catchFeat.get('mad_m3_pers'))
    ...
    water_elevation = ee.Algorithms.If(
        fsl_masl.eq(-999),
        ee.Number(dam_elevation).add(imputed_dam_height),
        fsl_masl
    )
    ...
    if debug_mode == True:
        ...
        print("\n [delineate_reservoir] dam_point_location", snapped_dam_point_location.getInfo())     
    ...
    imputed_water_elevation_prov_str = ee.Number(
            imputed_water_elevation_prov
        ).format("%.0f")
    
    dam_elevation_str = ee.Number(
            dam_elevation
        ).format("%.3f")
    
    water_elevation_str = ee.Number(
            water_elevation
        ).format("%.0f")
    
    reservoirVector = ee.Feature(reservoirVector).set("r_imputed_water_elevation", water_elevation_str)
    reservoirVector = ee.Feature(reservoirVector).set("r_imputed_water_elevation_prov", imputed_water_elevation_prov_str)
    reservoirVector = ee.Feature(reservoirVector).set("d_dam_elevation", dam_elevation_str)
...
def delineate_existing_reservoir(catchmentVector, c_dam_id_str, landcover_delineation_file_str):
    ...
    if debug_mode == True:
        print("\n [delineate_reservoir] raw_dam_point_location", raw_dam_point_location.getInfo())   
        print("\n [delineate_reservoir] snapped_point_location", snapped_dam_point_location.getInfo())           
        print("\n [delineate_reservoir] reservoirVector", reservoirVector.getInfo()) 
    
    water_elevation_str = "UD"
