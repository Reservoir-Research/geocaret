...
def vectorise_catchment(damFeat, watershedDptsFtc, c_dam_id_str) :
    ...
    c_polygons_ftc = ee.FeatureCollection(c_all_geometries_list.map(extract_polygons)).sort('area_m', False)
    ...
    c_outer_boundaries_list = ee.List(ee.List(c_outer_list).map(extract_outer_bound).get(0)) 
    ...
    c_boundaries_geom = c_outer_boundaries_geom.difference(**{'right': c_inner_boundaries_geom, 'maxError': 1})
...
def detected_watershed_pts(watershed_pt_indices, watershedCptsFtc, watershedGridFeat) :
    ...
    grid_id_list = (ee.Array(watershedGridFeat.get('grid_id'))
       .toList()
       .flatten()
    )
    ...
    #
    watershed_pt_ids = ( ee.List(watershed_pt_indices)
      .map(lambda e: grid_id_list.get(e))
    )
    
    # Keep points inside subcatchment  
    watershedDptsFtc = (watershedCptsFtc
      .filter(ee.Filter.inList("grid_id", watershed_pt_ids))
    ) 
...
def watershed_search_area(damFeat):
    ...
    search_area = (outlet_subcatch.geometry()
...
def watershed_search_grid(damFeat) :
   ...
   search_area_ft = (sdd_grid_coord
    .sampleRectangle(**{"region" : search_area ,
         "defaultValue": 999, 
   ...
   grid_id_list = (ee.Array(search_area_ft.get('grid_id'))
       .toList()
       .flatten()
...
def watershed_grid_index(damFeat, watershedCptsFtc, watershedGridFeat):
    ...
    snapped_point = (candidate_points
    ...
    for c_dam_id in c_dam_ids :
        ...
        try:
            ...
            if cfg.exportNiCatchmentVector == True:
                heet_export.export_ftc(ee.FeatureCollection(ee.Feature(niCatchFeat)), c_dam_id_str, "ni_catchment_vector")
    ...
    for c_dam_id in c_dam_ids :
        ...
        try:
            ...
            if cfg.exportWatershedCpts == True:
                heet_export.export_ftc(watershedCptsFtc, c_dam_id_str, "candidate_watershed_pts")
        ...
        try:
            ...
            if cfg.exportWatershedDpts == True:
                heet_export.export_ftc(watershedDptsFtc, c_dam_id_str, "detected_watershed_pts")
        ...
        try:
            ...
            if cfg.exportCatchmentVector == True:
                ...
                catchmentVectorFeat = catchmentVectorFeat.copyProperties(**{'source': damFeat, 'exclude' : ['ancestor_ids']})
    
                heet_export.export_ftc(ee.FeatureCollection(ee.Feature(catchmentVectorFeat)), c_dam_id_str, "catchment_vector")
