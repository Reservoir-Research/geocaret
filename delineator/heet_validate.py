...
def landcover_file_name(c_yr):
  ...
  c_yr_str = str(c_yr)
  return f"projects/ee-future-dams/assets/XHEET_ASSETS/ESACCI-LC-L4-LCCS-Map-300m-P1Y-{c_yr_str}-{dataset_version}"
...
def valid_fields(df: pd.DataFrame) -> Tuple[bool, str]:
    ...
    required_fields = ['id', 'country', 'name', 'dam_lat', 'dam_lon', "future_dam_model"]
...
def prepare_input_table(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['t_landcover_buffer_method'] = df.apply(lambda row: landcover_buffer_method(row['t_year_commissioned']),axis=1)
    df['t_landcover_analysis_yr'] = df.apply(lambda row: landcover_analysis_yr(row['t_year_commissioned']),axis=1)
    df['t_landcover_analysis_file'] = df.apply(lambda row: landcover_file_name(row['t_landcover_analysis_yr']),axis=1)
    df['t_landcover_delineation_yr'] = df.apply(lambda row: landcover_delineation_yr(row['t_year_commissioned'], row['future_dam_model']),axis=1)
    df['t_landcover_delineation_file'] = df.apply(lambda row: landcover_file_name(row['t_landcover_delineation_yr']),axis=1)
