...
def clear_tmp_folder():
    ...
    logger.info(f"[clear_tmp_folder] Found {str(len(asset_collection))} items in {cfg.ps_heet_folder}")
...
def assets_to_ftc():
    ...
    for c_dam_id in c_dam_ids :
        ...
        snapped_assets_list = ee.List(
            [ee.FeatureCollection(x['id']) for x in asset_collection if re.search(p_search_term, x['name']) ]
        )
        
        catchment_assets_list = ee.List(
            [ee.FeatureCollection(x['id']) for x in asset_collection if re.search(c_search_term, x['name']) ]
        )
        
        reservoir_assets_list = ee.List(
            [ee.FeatureCollection(x['id']) for x in asset_collection if re.search(r_search_term, x['name']) ]
        )        
    
        nicatchment_assets_list = ee.List(
            [ee.FeatureCollection(x['id']) for x in asset_collection if re.search(n_search_term, x['name']) ]
        )        

        river_assets_list = ee.List(
            [ee.FeatureCollection(x['id']) for x in asset_collection if re.search(s_search_term, x['name']) ]
        )      
...
def export_to_drive():
    ...
    try:
        ...
        logger.info(f"[export_to_drive] Found {str(len(asset_collection))} items in {cfg.ps_heet_folder}")
...
def export_to_assets():
    ...
    target_asset_folder = cfg.root_folder + "/" + cfg.heet_folder.split("/")[0] + "/" + cfg.output_asset_folder_name
    ...
    try:
        ...
        if len(asset_collection) > 0:
            ...
            for target_asset in assets_to_rename:
                asset_name = target_asset['name']
                new_asset_name = re.sub( cfg.ps_heet_folder, target_asset_folder, asset_name)
...
def kill_all_heet_tasks():
    ...
    running_heet_jobs = [
        t for t in task_list if (
            (t['metadata']['state'] not in ['SUCCEEDED', 'CANCELLED', 'FAILED']) and 
            (re.match("^XHEET-X\d\d\d-\d+", t['metadata']['description']))) 
    ]
    ...
    for task_log_name in task_log_names:
        ...
        for k, v  in  list(task_dict.items()) :
            ...
            try:
                ...
                logger.debug(f"[task_audit] There was a problem fetching task status for: {task_key}")                   
...
def run_analysis(pbar):
    ...
    t_landcover_buffer_method = dams_ftc.aggregate_array("t_landcover_buffer_method").getInfo()
    t_landcover_analysis_file = dams_ftc.aggregate_array("t_landcover_analysis_file").getInfo()
    t_landcover_delineation_file = dams_ftc.aggregate_array("t_landcover_delineation_file").getInfo()
    ...
    id_landcover_delineation_file_dict = dict(zip(c_dam_ids, t_landcover_delineation_file))
