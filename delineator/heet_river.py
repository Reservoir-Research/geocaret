...
def filter_main(hyriv_id, hydrorivers_extract):
    ...
    main_dam_reach_ft = hydrorivers_extract.filter(ee.Filter.eq("NOID", hyriv_id))
...
def find_main_channel(inundated_river_ftc, damFeat):
    ...
    main_dam_reach_id = main_dam_reach_ft.get("NOID")
...
def inundated_reaches(sres_ftc):
    ...
    reaches_sres_ftc = dta.FREEFLOWINGRIVERS.filterBounds(sres_ftc.geometry())

    def is_source(rfeat):

        # Source (Not in NDOID set)
        t_hyriv_id = rfeat.get("NOID")
    ...
    def is_sink(rfeat):
        ...
        # Sink (NDOID not in NOID Set)
        t_next_down = rfeat.get("NDOID")
    ...
    hyriv_id_set = reaches_sres_ftc.aggregate_array("NOID")
    next_down_set = reaches_sres_ftc.aggregate_array("NDOID")
