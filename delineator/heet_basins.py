...
def find_hybas_bounding_level(damFeat) :
    ...
    def algorithm(current, previous):
        ...
        outlet_subcatch_id = ee.Number(ee.Feature(outlet_subcatch.first()).get('HYBAS_ID'))
    ...
    bounding_level = ee.Number(
      ee.List(distinct_levels).get(-1)
    ).format("%s")
...
def get_ancestors(id, subcatchments) :
    ...
    if debug_mode == True:
        ...
        print("[DEBUG]\n [get_ancestors] Subcatchments size: " + subcatchments.size().getInfo())
    ...
    sources = (
        ee.Number(ee.List(nodes)
        .filter(
            ee.Filter.inList('item',ee.List(next_nodes)).Not()
        ).length())
    )
    ...
    def algorithm(current, previous) :
        ...
        parent_list = ee.List(child_list).map(lambda child :
            (subcatchments
                .filter(ee.Filter.eq('NEXT_DOWN', child))
                .aggregate_array('HYBAS_ID'))
            )
...
def batch_find_upstream_basins(dams_ftc, c_dam_ids):
    ...
    for c_dam_id in c_dam_ids :
        ...
        try:
            ...
            logger.exception("Finding highest hydrobasin level and bounding basin that encloses the catchment")
            continue
        ...
        try:
            ...
            dam_subbasin_feat = (dta.HYDROBASINS12
              .filter(ee.Filter.eq('HYBAS_ID', opt_12_hybasid))
            )
        ...
        try:
            ...
            SHYDROBASINS12 = (dta.HYDROBASINS12
                ...
                .filter(ee.Filter.stringStartsWith('SPFAF_ID', opt_bl_spfafid))
                .sort('SORT', True)
             )
            
        except Exception as error:
            logger.exception("Removing downstream sub-basins and those outside the bounding basin from search")
    
        if (cfg.upstreamMethod == 1) :
            try :
                logger.info("Finding upstream sub-basins by tracing ancestors")
                upstream_subbasins_list = get_ancestors(
                    opt_12_hybasid, SHYDROBASINS12
                )
            except Exception as error:
        ...
        if (cfg.upstreamMethod == 3) :
            ...
            try :
                ...
                UPSTREAM = SHYDROBASINS12.filter(ee.Filter.eq('ALL_ODD_OR_ZERO', ee.Number(1)))
            
                upstream_subbasins_list = ee.List(UPSTREAM.aggregate_array('HYBAS_ID'))
                
            except Exception as error:
                logger.exception("[ERROR] Identifying upstream sub-basins from pfafstetter ids")    
                continue 
      
        
        snappedDamFeat = snappedDamFeat.set('outlet_subcatch_id', opt_12_hybasid)
        snappedDamFeat = snappedDamFeat.set('ancestor_ids',  ee.String.encodeJSON(upstream_subbasins_list))
        ...
        if (cfg.exportSnappedDamPts == True) :  
            ...
            try:
                ...
                location_ftc = ftc.map(
                    lambda feat: ee.Feature(feat)
                )
                
                heet_export.export_ftc(location_ftc, c_dam_id_str, "snapped_dam_location")
            except Exception as error:
              logger.exception(f'{msg} {c_dam_id_str}')
              continue
