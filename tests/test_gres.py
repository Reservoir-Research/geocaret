...
def test_mean_slope_perc_gres():
    ...
    W_12_Catchment = ee.FeatureCollection("projects/ee-future-dams/assets/XHEET_TEST_POLYS/W_12_Catchment")
...
def test_mghr_gres():
    ...
    W_12_Reservoir = ee.FeatureCollection("projects/ee-future-dams/assets/XHEET_TEST_POLYS/W_12_Reservoir_Annotated")
    ...
    logger.info(f'[mghr] MGHR - HEET {calc_result} ' + 
                f'Manual {test_result} Diff {diff} PDiff {pdiff}')     
...
def test_mean_annual_runoff_mm_gres():
    ...
    logger.info(f'[mean_annual_runoff_mm] Runoff - HEET {calc_result} ' + 
                f'Manual {test_result} Diff {diff} PDiff {pdiff}')     
...
def test_population_gres():
    ...
    logger.info(f'[population] Population Count - HEET {calc_result_1} ' + 
                f'Manual {test_result_1} Diff {diff1} PDiff {pdiff1}')     

    logger.info(f'[population] Population Density - HEET {calc_result_2} ' + 
...
# @pytest.mark.skip
def test_maximum_depth_alt1_gres():
...
def test_mean_depth_gres():
    ...
    logger.info(f'[mean_depth] Mean Depth - HEET {calc_result} ' + 
                f'Manual {test_result} Diff {diff} PDiff {pdiff}')     
