...
def update_sp_err_folder(sp, folder_path):
    sp.write("")
    sp.write("  [ERROR] HEET encountered an error and will exit")
    sp.write(f"  [ERROR] Folder {folder_path} already exists. Please fix this and try again")   
    sp.write("")    
    sp.write("Thank you for using HEET!")    
    return(sp)
...
def update_sp_err_running(sp)  :
   #sp.write("123456789|123456789|123456789|123456789|123456789|123456789|123456789|123456789|")
    sp.write("")
    sp.write("  Oops! Your Google Earth Engine Asset Task List already contains HEET tasks")
    sp.write("        It looks like another HEET analysis is already running")
    sp.write("")
    sp.write("        If you choose to continue, any existing HEET tasks (with a description")
    sp.write("        starting 'XHEET-X') will be cancelled and the contents of Asset Folder")  
    sp.write("        XHEET/tmp will be deleted")    
    sp.write("")        
    sp.write("        You will be asked to confirm that you understand this msg and then") 
...
def update_sp_err_interrupted(sp):  
    sp.write("")
    sp.write("  Oops! Your Google Earth Engine Asset Folder XHEET/tmp already contains some")
    sp.write("        files. It looks like a previous HEET analysis was interrupted and did") 
...
def update_sp_inv_output(sp, issues):          
   #sp.write("123456789|123456789|123456789|123456789|123456789|123456789|123456789|123456789|")    
    sp.write("")
    sp.write("  [Warning] Calculated parameters contain impossible or improbable values on") 
...
def update_sp_err_time(sp):
    ...
    sp.write("  [WARNING] Analysis wait time limit exceeded. Cancelling all unfinished HEET") 
...
def main():
    ...
    with yaspin(Spinners.line, text= step_desc, color="yellow") as sp:
        ...
        # Fields valid
        try:
            status = heet_validate.valid_fields(df_inputs)
        except Exception:
            sp = update_sp_fail(sp)              
        ...
        # File valid
        try:
            status = heet_validate.valid_input(df_inputs, input_file_path, output_folder_path)
        except Exception:
            sp = update_sp_fail(sp)              
            sp = update_sp_err_fatal(sp)
            sys.exit()
    ...
    with yaspin(Spinners.line, text=step_desc, color="yellow") as sp:
        ...
        try:
            ...
            if (existing_tasks == True) or (existing_files == True):
                ...
                # Followup questions
                answer1 = prompt([questions[0]], style=style, raise_keyboard_interrupt=False)
                if answer1=={}:
                    sys.exit("Exiting (Keyboard Interrupt)")                           
                if (answer1["understand"] == False ) :
                    sys.exit("Exiting (Did not understand)")                
                answer2 = prompt([questions[1]], style=style, raise_keyboard_interrupt=False)
                if  answer2=={}:
                    sys.exit("Exiting (Keyboard Interrupt)")  
    ...
    with yaspin(Spinners.line, text = step_desc, color="yellow") as sp:
        ...
        try:
            ...
            sp = update_sp_fail(sp)  
            sp = update_sp_err_fatal(sp)
            logger.exception("[heet_cli] There was a problem preparing input file.  Exiting...")                
            sys.exit()                  
        ...
        try:
            ...
            sp = update_sp_fail(sp)              
            sp = update_sp_err_fatal(sp)
            logger.exception("[heet_cli] There was a problem converting input file from df to ftc.  Exiting...")                
            sys.exit()       
        ...
        try:
            ...
            logger.exception("[heet_cli] There was a problem uploading user inputs to Google Earth Engine. Exiting...")            
            sys.exit() 
        ...
        try:
            ...
            sp = update_sp_fail(sp)              
            sp = update_sp_err_upload(sp)
            logger.info("[heet_cli] Uploading user inputs to Google Earth Engine timed out. Exiting...")      
            sys.exit()             
        except Exception:
            sp = update_sp_fail(sp)              
            sp = update_sp_err_upload(sp)
        ...
        try:
            ...
            sp = update_sp_fail(sp)  
            sp = update_sp_err_upload(sp)
            logger.debug("[heet_cli] There was a problem uploading user inputs to Google Earth Engine. Exiting...")
    ...
    with yaspin(Spinners.line, text = step_desc, color="yellow") as sp:
        ...
        try:
            ...
            if status == 1:                
                ...
                logger.info("(Running) Analysis wait time limit exceeded. Cancelling all unfinished HEET tasks.")                  
                try:
                    heet_task.kill_all_heet_tasks()   
                except Exception:
                    sp = update_sp_err_fatal(sp)
    ...
    with yaspin(Spinners.line, text = step_desc, color="yellow") as sp:
        ...
        try:
            ...
            sp = update_sp_fail(sp)             
            sp = update_sp_err_time(sp)
            logger.info("(Waiting) Analysis wait time limit exceeded. Cancelling all unfinished HEET tasks.")  
            try:
                heet_task.kill_all_heet_tasks()   
            except Exception:
                sp = update_sp_err_fatal(sp)           
    ...
    with yaspin(Spinners.line, text = step_desc , color="yellow") as sp:
        ...
        try:
            ...
            try:
                status = heet_validate.valid_output(df_outputs, output_file_path, output_folder_path)     
            except Exception:
                # Handles any issue, including connectivity
                sp = update_sp_fail(sp)              
                sp = update_sp_warn_skip(sp)
    ...
    #==========================================================================  
        ...
        sp.write("")     
        sp.write("  Please right click and follow the link below to view your results in Google drive:")
        sp.write(f"  https://drive.google.com/drive/u/0/search?q={cfg.output_drive_folder}")
        sp.write("")  
        sp.write("  Please follow the link below and navigate to the Assets tab to view your results in Google Earth Engine:")
        sp.write(f"  https://code.earthengine.google.com/")
        sp.write("")   
        sp.write("  Please check the following local folder for output parameters (csv/json) and input/output validation reports")
        sp.write(f"  outputs/{output_folder_name}")
        sp.write("")               
        sp.write("Visualisation:")  
        sp.write("")    
        sp.write("  To visualise your results, please navigate to the following script and update the parameters as directed:")
        sp.write(f"  https://code.earthengine.google.com/264776718cccc3c6df5295200185d0a1")
        sp.write("")  
        sp.write("Thank you for using HEET!")                              
